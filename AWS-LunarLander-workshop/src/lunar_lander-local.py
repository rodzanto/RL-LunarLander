...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        # --------------------------------- KINEMATICS DEFINED HERE ------------------------------------
        # Do not touch code from here ....
        # ----------------------------------------------------------------------------------------------
        if self.continuous:
...
#         # SCENARIO 2 - Most centered landing
#         c2 = 500   
            
#         # SCENARIO 3 - Most level landing
#         c3 = 500            
            
#         # SCENARIO 4 - Minimum fuel
#         c4 = 0.5
        ...
        shaping = \
            - c1*np.sqrt(state[2]*state[2] + state[3]*state[3]) \
            - c2*np.sqrt(state[0]*state[0] + state[1]*state[1]) \
            - c3*abs(state[4]) + 10*state[6] + 10*state[7]   # And ten points for legs contact, the idea is if you
        ...
        reward -= m_power*c4  # less fuel spent is better, about -30 for heurisic landing
        reward -= s_power*(c4/10.0)
        
        # DO NOT TOUCH
        
        self.fuel = self.fuel + m_power + s_power
#         print('dist_from_center={};'.format(str(np.sqrt(state[0]*state[0] + state[1]*state[1]))))
#         print('vel_at_end={};'.format(str(np.sqrt(state[0]*state[0] + state[1]*state[1]))))
#         print('angle_at_end={};'.format(str(abs(state[4]))))
#         print('fuel_used={};'.format(str(self.fuel)))
    ...
    def render(self, mode='human'):
        ...
        drawer = OpencvDrawFuncs(w=VIEWPORT_W, h=VIEWPORT_H, ppm=20)
        drawer.install()
        drawer.clear_screen()
        drawer.draw_world(self.world)
        return drawer.screen[:,:400,:]
