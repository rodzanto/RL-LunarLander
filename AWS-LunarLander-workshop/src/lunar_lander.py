...
from reward_function import my_reward_function
...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        reward, self.prev_shaping = my_reward_function(state, m_power, s_power, self.prev_shaping)
        
        # DO NOT TOUCH
        
        self.fuel = self.fuel + m_power + s_power
        print('dist_from_center={};'.format(str(np.sqrt(state[0]*state[0] + state[1]*state[1]))))
        print('vel_at_end={};'.format(str(np.sqrt(state[2]*state[2] + state[3]*state[3]))))
